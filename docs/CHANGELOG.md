...
最終更新: 2026-04-30（ルール11 追加）

---

## 2026-04-30 [village-tsubasa] RULES.md ルール11「Hosting メニュー 404 修正フロー」を追加

- `docs/RULES.md` にルール11を追加
  - Step 1〜7 の runbook（網羅的な原因特定 → 実体特定 → 方針選択 → 実装 → preview デプロイ → 動作確認 → 本番デプロイ → CHANGELOG 記録 → commit/push）
  - 手本実装として `public/expense/index.html` を参照
  - **禁止事項**: preview を飛ばしていきなり本番デプロイ
- 同日 `/expense/` の 404 修正で確立した手順を runbook 化し、次回の `/feedback/` 修正等で Claude が同じ流れを自動的に踏めるようにするのが狙い
- **影響範囲**: ドキュメント追記のみ。コード・API・Supabase・他アプリへの影響なし
- 関連コミット: （後続コミットで追記）
