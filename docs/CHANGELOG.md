...
最終更新: 2026-08-10（qrec の参考記録に sub2 の記録を含める）

---

## 2026-08-10 [village-tsubasa] AI下書きの参考例（samples）を旧DB と sub2 の両取りに変更

**症状**: 2026-08 以降の記録は sub2 に保存されるが、参考記録 API は旧DB しか見ていなかったため、8月分が AI 下書きの参考例（qrec の「参考記録」欄）に一切入らなかった。HANDOFF §10-1 に「未実装」として記載されていた項目。

### 変更内容

- `functions/src/service-records-home/samples.ts` / `functions/src/service-records-move/samples.ts`
  - 旧DB（`service_notes_home` / `service_notes_move`）と sub2（`service_records_home` / `service_records_move`）に**同じ条件で並行問い合わせ**し、`service_date` 降順でマージしてから既存どおりシャッフル → 最大10件返す
  - sub2 側は実装済み・未使用だった `lib/serviceRecordsSub2.ts` の `fetchSub2HomeSamples` / `fetchSub2MoveSamples` を流用
  - **期間分割（`isSub2Date`）は使わない**。参考例は「過去の書きぶり」が目的で保存先とは無関係であり、期間で振り分けると境界をまたいだ瞬間に母集団が片方だけになって手本が痩せるため
  - 片側のDBが落ちても、成功した側の参考例は返す（`console.error` は必ず出す）。両方失敗したときだけ 500
- 旧DB 側の `user_name` 突合を、**検索キーが「様」で終わるときだけ前方一致**（`like 'X%'`）に変更。それ以外は従来どおり完全一致

### user_name の表記ゆれ（前方一致にした理由）

旧DB の `user_name` には区分サフィックス付きが **11件**混在する（実データで確認。全て `(通院)` か `(家事)`）が、sub2 は**サフィックスなしで統一**されている。

- 完全一致のままだと、**サフィックス付きでしか記録が無い利用者**（岩下遥香様・田中龍気様）の過去記録が1件も取れない
- 「様」は敬称の終端なので「◯◯様…」で始まる別人は構造上あり得ず、前方一致でも巻き込み事故が起きない。敬称なしの姓での前方一致（「田中」が「田中龍気様」を拾う）を避けるため「様」条件を付けている
- サフィックスを列挙して除去する案は不採用。新しい区分が増えた瞬間に**無音で取りこぼす**（＝今回の8月分欠落と同じ壊れ方をする）ため
- `delayNotify.ts:577` の `normalizeName()` は空白除去と末尾「様」除去のみで、括弧サフィックスには効かない（通すと逆に不一致が確定する）ため流用しなかった

**未解決**: 異体字ゆれ（`門崎` / `門﨑`）は前方一致では拾えない。現状は両DBの表記が一致しており実害はないが、片方だけ変わった時点で無音で取りこぼす。**別件として切り出し**。

### 影響範囲

- **本リポ内のみ。** レスポンス形式（`{ ok, samples: [{service_date, task, note}] }`）は不変のため、フロント（`public/qrec-okuhara-9k2b/main.js`）の改修は不要
- スキーマ変更なし。sub2 への書き込みなし。**読み取りのみの変更**
- `previous.ts` は**凍結**（呼び出し元ゼロ、返している値が記録本文ではなく `home_schedule_tasks.summary` ＝ 予定概要のため）。HANDOFF §10-1 の「未実装」からは外れない
- 呼び出し元は `/qrec-okuhara-9k2b/`（オーナー記録画面）のみ。ヘルパー用の記録画面は samples を呼んでいない

### 検証

- `npm run build --prefix functions` exit=0、既存テスト 9件 pass / 0 fail
- **実データでの動作確認は未実施**（デプロイ後に qrec で8月分が参考例に出るかの確認が必要）

- 関連コミット: 後述（`feat/qrec-samples-sub2`）
