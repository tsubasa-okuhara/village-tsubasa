...
最終更新: 2026-09-13（重度訪問介護の実施項目を専用化 — 身体介護の暫定流用を解消）

---

## 2026-09-13 [village-tsubasa] 重度訪問介護の実施項目を専用化（身体介護の暫定流用を解消）

2026-09-08 に「専用UIを後日作るまでの暫定」として身体介護の項目を流用していた件の解消。
対象は `public/service-records-home/main.js` と `public/service-records-home/index.html` のみ。

- **方針**: 重訪の実施項目を `HOME_JUDO_ITEM_GROUPS` として独立させ、
  village-admin の「サービス提供記録」様式（`excel-home.ts:388-410`）の
  重訪セクション B28「重度訪問介護（※上記のチェック欄も使用）」と同じ組み立てにした。
  身体・家事の欄も併用したうえで外出介助を**追加**する様式なので、
  「移動、重訪」の日でも身体・家事を隠す切り替えにはしない。
- **グループ構成**（表示順）:
  - 身体 — 身体介護から流用（子項目・食事量ラジオもそのまま）
  - 家事 — 調理 / 掃除 / 洗濯 / 買い物（**独立した主項目**にした）
  - 重度訪問介護 — コミュニケーション支援 / 相談・助言 / 見守り（常に表示）
  - 外出・移動 — 外出介助（**task 原文に「移動」を含むときだけ表示**）
  - （見出しなし）その他
- **身体介護からの差分は2つだけ**:
  - `移動介助` を外し、外出・移動グループの `外出介助` に統合
    （`外出介助` は様式 D29 の文言。旧ラベルは身体介護欄 B23 の語）
  - `利用者とともに行う家事`（子: 掃除/洗濯/調理）の入れ子をやめて4項目を独立
- **外出・移動の判定**: `requiresOutingSection(task)` = `task.includes("移動")`。
  village-admin の移動介護欄（U列/AI列）の判定
  `dashboard/jisseki-source.ts:86` `(r.task ?? "").includes("移動")` と**同じ規則**。
  別リポなので実体は共有できない。片方を変えるならもう片方も見ること。
- **変更した関数**:
  - 新規 `HOME_JUDO_ITEM_GROUPS` / `requiresOutingSection` / `usesJudoItems`
  - `renderChecklist` から主項目1件ぶんのマークアップを
    `buildBodyCarePrimaryItemHtml(item, index)` に切り出し、重訪は
    `buildJudoChecklistHtml(task)` が**チェックリスト全体で一意な通し番号**を振って再利用。
    グループごとに index を振り直すと `home-body-primary-0` が複数でき、
    `<label for>` が別のチェックボックスを開閉し食事量ラジオの `name` も合体する
    （どちらも例外を出さず黙って誤動作する）。
  - `index.html` のインライン `<style>` に `.nested-checklist__section` /
    `__section-title` を追加（`style.css` は 0 バイトの空ファイルのため使っていない）。
- **触っていないもの**: 身体介護 / 家事援助 / 通院の項目、`generateSummary.ts` の
  AI要約プロンプト、保存経路（`task` を送らない・2026-09-13 デプロイ済み）。
  `usesBodyCareForm` から重訪を**外していない**（外すと `buildMemoText` が平坦な
  「実施項目: …」書式に落ち、village-admin の `parseMemo` が読む
  「主チェック:」「子チェック:」が出なくなって帳票が空になる）。
- **実測（node ハーネス、ブラウザ無し）**:
  - 身体介護のレンダリング結果が HEAD と**バイト一致**（16843 文字）。回帰なし。
  - id 重複 0（移動なし46 / 移動あり47 / 身体介護44）、食事量ラジオの `name` グループは各1つ、
    `<label for>` は全件が実在する input を指す。
  - 重訪の主項目 19 件（移動なし）/ 20 件（移動あり）。差分は `外出介助` の1件のみ。
- **影響範囲**: 本リポ内のみ + `village-admin` の帳票の**チェック位置**（下記の残課題）。
  スキーマ変更なし。API 変更なし。`user-schedule-app` への影響なし。
- **ルール4 の事前共有**: 2026-09-13 に奥原さん承認済み。

### 🔴 残課題: サービス提供記録 Excel を sub2 に向けるときに**同時に**直すもの

`village-admin/functions/src/dashboard/export-home.ts:18` は旧DB `service_notes_home` のみを読む。
**いまは壊れない。Excel を sub2 に向けた瞬間に顕在化する。**

実測（奥原さん、2026-09-13）:
- 旧DB `service_notes_home` は全 528 件、うち `service_date >= '2026-08-01'` は **0 件**
- → アプリ保存の重訪記録はこの Excel に**到達していない（確定）**

直すもの3点:

① **`hasCheck` が区分を見ていない**（`excel-home.ts:148`）
   ```ts
   const hasCheck = (name: string) => parsed.mainChecks.some(c => c.includes(name));
   ```
   主チェックの語を部分一致で見るだけなので、重訪の主チェックが
   **家事援助の列**に飛ぶ:
   | 主チェック | 飛び先 |
   |---|---|
   | 掃除 | D13「□掃除」= 家事援助欄 |
   | 洗濯 | D16「□洗濯」= 家事援助欄 |
   | 買い物 | D25「□買い物」= 家事援助欄 |
   | 調理 | 様式キーが「一般的な調理」のため `"調理".includes("一般的な調理")` = false → **黙って消える** |

② **重訪セクション4行が `□` 固定**（`excel-home.ts:388-410`）
   `checkBox(..., hasCheck(...))` に包んでいないため、主チェックを出しても反映されない。
   - B29 `□コミュニケーション支援`
   - D29 `□外出介助`
   - B30 `□生活等に関する相談、助言`
   - B31 `□常時付添いを必要とする見守り的援助`

③ **キー文字列は画面の短いラベル側に合わせる**
   `hasCheck` は部分一致なので、様式の長い文言ではなく画面のラベルを渡す:
   `hasCheck("コミュニケーション支援")` / `hasCheck("相談")` / `hasCheck("見守り")` / `hasCheck("外出介助")`

**未確認**: 本番にデプロイされている village-admin が、ここで読んだ
`feat/billing-sub2`（worktree `.claude/worktrees/july-billing-straddle`、
origin/main より先行）と同じかは git からは判断できない。

---

## 2026-09-08 [village-tsubasa] 居宅記録の「区分」をヘルパー入力から自動導出に変更（区分ラジオ廃止）

Notion「2026-07-26 ヘルパー向けサービス記録 記載基準対応（③）」の「② 居宅UI整理」。
移動側（2026-08-01・3セクション削除）に続く居宅側の整理。

- **方針**: 精査済みの `task` を正とし、ヘルパーは区分を選ばない。UI は自動表示のみ。
- **変更**:
  - `index.html` の区分ラジオ3択（`#home-category-group`）を削除し、
    readonly の `#home-category-display` + `#home-category-hint` に置換。
    未使用になった `.segmented-control` の CSS も削除（HTML/JS/CSS をセットで落とす）。
  - `main.js`: `state.selectedCategory`（ヘルパーの選択）と `state.inferredServiceType` を
    `state.category` 1本に統合。`inferServiceType(task)` の結果をそのまま使う。
    change リスナー・`clearCategoryRadios`・`toCategoryChoice` を削除。差し引き **-101行**。
  - `isBodyCareCategory` → `usesBodyCareForm`。入れ子フォームを使う区分を
    `身体介護` と `重度訪問介護` の2つに。
- **memo の「区分: 」に出る値を3種類 → 4種類に拡張**（`重度訪問介護` を追加）。
  **ルール4 の事前共有対象。2026-09-08 に奥原さん承認済み。**
  - 従来は重訪も `身体介護` と記録されていた（実績記録票のチェックが身体介護に付く）。
  - `village-admin` は**無変更**。`excel-home.ts:208-212` の
    `cat.includes("身体"/"家事"/"通院"/"重度")` に対し、4値がそれぞれ
    **ちょうど1分岐**に当たることを確認済み（`重度訪問介護` は身体/家事/通院を含まないため
    `isJudo` だけ true）。既にあるが未到達だった `isJudo` 分岐に初めて到達する。
- **判定不能（`移動` 単独・空・`GH〜外出〜GH` など）は保存不可**にした。
  ヒントに「事業所に連絡してください」を表示し、保存と AI 要約を止める。
  既定の区分に倒すと 2026-09-07 に直した不具合（誤った区分が memo と帳票に入る）を再現するため。
  - 転送側 `サービス記録転送.js:1100-1110` が判定不能の居宅行でバッチごと中断し
    （`:1260-1268` で `return {stopped:true}`、書き込み `:1335` はその後）、
    `service_records_home` に到達しない。**本来起きない前提のフェイルセーフ**。
  - `inferServiceType` のキーワードと順序は転送側 `HOME_SERVICE_CONTENT_RULES` と同一。
  - ⚠️ 2026-08-20 以前にこのゲートが存在したかは**不明**（当該 GAS の git 履歴は 08-21 開始）。
    精査シートを経由しない手動 INSERT があった場合も対象外。
- **`task` は引き続き一切書き込まない**（2026-09-07 の修正を維持）。
- **影響範囲**: 本リポ内のみ + `village-admin` の帳票の**チェック位置**（コード変更は不要）。
  スキーマ変更なし。API 変更なし。`user-schedule-app` への影響なし。
- **⚠️ 新規に判明した別件**: `village-admin/functions/src/dashboard/export-home.ts:18` は
  **旧DB `service_notes_home` しか読んでいない**（sub2 への分岐なし）。
  このため 2026-08 以降にアプリで保存した記録は**実績記録票 Excel に出ていない**。
...
作業ではなく**記録のみ**。9/10 の提出前に環境を変えたくないため、今回は削除も移動もしていない。

`~/.claude/CLAUDE.md` §4 のパス記述を実地確認した際に、同名リポジトリが2箇所にあることが判明。

| リポジトリ | `~/` 直下（正・作業用） | `~/Desktop/デスクトップ - Unknown/`（古いクローン） |
|---|---|---|
| `village-tsubasa` | `fc01cca` 2026-09-07 / 未コミット0 | `c7eabdd` 2026-05-20 / 未コミット0 |
| `village-admin` | `1bf2120` 2026-08-29 / 未コミット1 | `466ec67` 2026-05-06 / 未コミット5 |
| `user-schedule-app` | `22fa8c0` 2026-04-18 / 未コミット1 | `eb186a1` 2026-05-29 / 未コミット7 |

- **削除可能と確認済み**（2026-09-08 奥原確認）。`user-schedule-app` の Desktop 側は
  `~/` 直下より新しい（05-29 > 04-18）が、未コミット7件の内訳は
  `.DS_Store` 2件・`.gitignore`・`.bak`/`.backup` 4件で**すべて未追跡のゴミ**。
  拾うべき編集はなく、`origin/main` とも同期済み。
- **実施は 9/10 の提出後**。それまでは触らない。
- `admin-approval` は Desktop 配下が唯一の実体なので**対象外**（消さないこと）。
- 影響範囲: ローカル環境のみ。3アプリの動作・スキーマ・API に影響なし。

---

## 2026-09-07 [village-tsubasa] 居宅サービス記録の保存で `service_records_home.task` を上書きしないよう修正

- **事象**: ヘルパーが居宅の記録を保存すると、GAS が転送時に書いた `task` の原文
  （`身体` / `家事` / `重訪` / `移動、重訪` など）が、画面の「区分」ラジオ3択
  （`身体介護` / `家事援助` / `通院等介助・通院等乗降介助`）で上書きされていた。
  上書きは初回保存の1回だけ（2回目以降は `already_written` で 409）。
- **影響**: 4900（重度訪問介護）の移動介護加算は `task` 原文の「移動」の有無で判定しているため、
  保存すると加算判定の入力が消える。2026-09 の実データ35件で、保存済みの行はすべて3択の値に化けていた。
  9/3 の1件で移動介護1回（100単位・1,120円）が落ちかけた。
- **修正**:
  - `task` を保存経路から全廃（フロント payload / API の受け口 / `Sub2HomeSavePayload`）。
    API は body に `task` が来ても無視するので、古いタブや `qrec-okuhara-9k2b` 画面からの
    保存でも上書きされない。
  - `inferCategory` → `inferServiceType` に置換。原文が短縮表記なので部分一致にし、
    判定順を転送側 `HOME_SERVICE_CONTENT_RULES` と揃えた（**重訪/重度 を先に見る**。
    「重度な身体介護」を身体と誤判定しないため）。旧実装は完全一致で、実際の原文には
    1件も一致せず全部が既定値 `身体介護` に倒れていた。
  - 判定不能・重度訪問介護は `身体介護` に倒さず**ラジオ未選択**にし、区分欄の下に
    推定結果を表示して選択を促す（`#home-category-hint` を新設）。保存前バリデーションで止まる。
  - `home-task`（サービス内容欄）に予定の原文を表示（従来は区分の値で上書き表示していた）。
- **memo は変更なし**: `区分: ` に出る値は従来の3種類のまま。village-admin の
  `parseMemo`（`functions/src/dashboard/excel-home.ts`）が読む形式・値域を変えていない。
- **影響範囲**:
  - village-admin: 変更なし。ただし `excel-home.ts:207` は `parsed.category || rec.task` の
    フォールバックがあり、`task` が原文に戻ることで挙動が変わる可能性がある
    （memo に `区分: ` がある記録では従来どおり memo 優先なので、通常は不変）。
    加えて `isJudo = cat.includes("重度")` は原文の `重訪` にヒットしない。**別途要確認**。
  - user-schedule-app: 影響なし。
  - スキーマ変更なし（列の追加・削除・型変更はしていない）。
- **既存データ**: 上書き済みの記録は本修正では戻らない。9月分の `task` 原文の復元は別タスク。
- **関連**: Notion「2026-07-26 ヘルパー向けサービス記録 記載基準対応（③）」の
  「② 居宅UI整理」。区分ラジオの削除は今回**見送り**（表示とチェックリスト出し分けに使うため残置）。
- **残課題（別タスク・今回は着手しない）**:
  1. **`qrec-okuhara-9k2b` の `inferCategoryFromTask` の整理**（`public/qrec-okuhara-9k2b/main.js:489`）。
     同画面の `saveHome`（`:725`）は同じ `/service-records-home/save` に
     `task: state.selectedCategoryValue || task.task` を送り続けているが、
     **API 側が `task` を無視するので DB は守られている**。
     残る影響は `final_note` の文面のみ。判定の中身は居宅画面と別物で、
     短縮表記に非対応・`重訪` を見ない・戻り値が `通院等介助`（居宅画面の
     `通院等介助・通院等乗降介助` と別文字列）。整理するなら送信側の `task` も併せて落とす。
     2026-09-07 時点で**意図的に据え置き**（奥原判断）。
  2. **village-admin `excel-home.ts:212` の `isJudo = cat.includes("重度")` の確認**。
     GAS 原文の `重訪` にはヒットしないため、`parsed.category || rec.task` の
     フォールバックで原文が使われる記録では重度訪問介護のチェックが付かない可能性。
  3. **9月分の上書き済み `task` の復元**。本修正では戻らない。
     精査シート側との突き合わせが必要。「移動」を含む行の洗い出しが先。
...
## 2026-05-11 [village-tsubasa] 経費アプリ — 取引先フィールドを自由入力テキストに変更

### Fixed
- 詳細・編集画面の取引先フィールドを `st.selectbox` → `st.text_input` に変更(app.py)
- OCR誤認識を直接修正できないという現場からの報告に対応
- 既存の取引先一覧は折りたたみ式エクスパンダー内に参照リストとして残置

### 影響範囲
- 本リポ内のみ（village-tsubasa の編集UIのみ。DBスキーマ・他アプリへの影響なし）

---

## 2026-05-10 経費アプリ Phase 1A — サイドバー月別統計追加

### Added
- サイドバー「あなたの統計」直下に **今月 / 先月の件数+合計** を表示(app.py)
- ヘルパーが「今月いくら使ったか」を全ページから一目で確認可能に

### Technical
- 既存 `get_receipt_stats(helper_email, date_from, date_to)` を月境界で2回呼び出し
- DB 変更なし、新規依存なし、~25行追加
- transaction_date インデックス利用で高速
