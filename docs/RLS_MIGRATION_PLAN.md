...
最終更新: 2026-04-25 夜（**全フェーズ完了 ✅**）
起草者: 奥原翼 + Claude Opus（本計画）

## ✅ 完了ステータス（2026-04-25 終了時点）

- ✅ Phase 0（診断）完了 — 全 27 オブジェクトの状態把握済み
- ✅ Phase 1（Group A 17テーブル RLS ON）— **適用完了**
  - 初回失敗 → village-admin Firebase Secret 修正 → 再適用で成功
- ✅ Phase 2（user-schedule-app コード調査）完了
- ✅ Phase 3（Group B 4テーブル RLS ON、選択肢A）— **適用完了**
- ✅ 全 4 アプリ（利用者・ヘルパー2画面・管理）の動作確認 OK

### 副次成果（隠れバグ修正）
- village-admin の Firebase Secret `SUPABASE_SERVICE_ROLE_KEY` に anon キーが
  保存されていた問題を発見 → service_role キーで上書き → Functions 再デプロイ
- 詳細は `CHANGELOG.md` 2026-04-25 のエントリ参照
- 教訓を `RULES.md` ルール7 に追記済み

### 残課題（Phase 4 候補、急がない）

1. 利用者アプリの認証導入（現状は anon 全許可で「警告は消えたが実質公開」状態）
2. `client_users` テーブルの実態調査と適切なポリシー設計
3. 未文書化テーブル `helper_priority` / `process_log` の用途調査
4. 電子契約 `contracts` 5テーブルを Supabase に作成（`sql/create_contracts.sql`）
