export const SOURCE_TYPE_MOVE = "move";

export const STRUCTURED_OPTIONS = {
  sourceTypes: [SOURCE_TYPE_MOVE],
...
export function isAllowedValue(allowedValues: readonly string[], value: string): boolean {
  return allowedValues.includes(value);
}

export function areAllowedValues(allowedValues: readonly string[], values: string[]): boolean {
  return values.every((value) => isAllowedValue(allowedValues, value));
}

export function getAllowedActionDetails(actionType: string): string[] {
  return STRUCTURED_OPTIONS.actionDetailsByType[actionType] ?? [];
}
