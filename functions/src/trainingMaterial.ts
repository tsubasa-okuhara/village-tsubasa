...
): Promise<{ items: Array<{ id: number; text: string }>; summary: string }> {
...
${trimmed}`;
  ...
  let parsed: { summary?: string; items?: Array<{ id?: number; text?: string }> };
  try {
  ...
  const normalized: Array<{ id: number; text: string }> = [];
  for (let i = 0; i < 5; i += 1) {
    const src = items[i];
    const text =
      src && typeof src.text === "string" ? src.text.trim() : "";
    normalized.push({
      id: i + 1,
      text: text || `学習項目${i + 1}`,
    });
 ...
 */
export async function handleCreateTrainingMaterial(
  req: Request,
  res: Response,
) {
  try {
    ...
    let checklistItems: Array<{ id: number; text: string }>;
    let summary = "";
    if (Array.isArray(manualItems) && manualItems.length === 5) {
      checklistItems = manualItems.map((it: { id?: number; text?: string }, i: number) => ({
        id: i + 1,
        text: (it?.text ?? "").toString().trim() || `学習項目${i + 1}`,
      }));
    } else {
      try {
        const gen = await generateChecklistItems(
          trainingName.trim(),
 ...
 */
export async function handleListTrainingMaterials(
  req: Request,
  res: Response,
) {
  try {
 ...
 */
export async function handleGetTrainingMaterial(
  req: Request,
  res: Response,
) {
  try {
 ...
 */
export async function handleDeleteTrainingMaterial(
  req: Request,
  res: Response,
) {
  try {
 ...
 */
export async function handleUpdateTrainingMaterial(
  req: Request,
  res: Response,
) {
  try {
    ...
    if (trainingDate) patch.training_date = trainingDate;
