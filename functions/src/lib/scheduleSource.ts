 ...
 */
function getCutoverYearMonth(): { year: number; month: number } {
  return {
...
export function isSub2YearMonth(year: number, month: number): boolean {
...
export function isSub2Date(date: string): boolean {
...
export function getCutoverStartDate(): string {
 ...
 */
export function formatClockTime(raw: string | null): string | null {
...
export interface Sub2HelperRow {
  helper_name: string | null;
  /** ログインメール。nullable（未登録のヘルパーがいる） */
  email: string | null;
}

/** sub2 の helper マスタ（35行規模）。件数が小さいので全件取ってアプリ側で突合する */
export async function fetchSub2Helpers(): Promise<Sub2HelperRow[]> {
 ...
 */
export function findSub2HelperNames(helpers: Sub2HelperRow[], helperEmail: string): string[] {
...
export function buildSub2EmailByHelperName(helpers: Sub2HelperRow[]): Map<string, string> {
...
export async function fetchSub2HelperNamesByEmail(helperEmail: string): Promise<string[]> {
  return findSub2HelperNames(await fetchSub2Helpers(), helperEmail);
}

// ========== 3. sub2 の schedule_entries ==========

export interface Sub2EntryRow {
  id: number;
  date: string;
  helper_name: string | null;
...
const SUB2_ENTRY_COLUMNS =
  "id, date, helper_name, user_name, start_time, end_time, transport, support_flow, helper_note";
 ...
 */
export async function fetchSub2EntriesByDate(date: string): Promise<Sub2EntryRow[]> {
  const supabase = getSupabaseSub2Client();
  ...
  const rows = (data ?? []) as Sub2EntryRow[];

  if (rows.length >= SUB2_ROW_LIMIT) {
    console.error(
      `[schedule-source] schedule_entries の取得が上限 ${SUB2_ROW_LIMIT} 件に達しました date=${date}。件数が切れている可能性があります`,
    );
  }
...
export interface ScheduleItemCore {
  id: string | number;
...
export interface ScheduleItemWithCoHelpers extends ScheduleItemCore {
  /** 同 (user_name, start_time) の他ヘルパー名（合同シフト用） */
  coHelpers: string[];
}
 ...
 */
export function toScheduleItemFromSub2(row: Sub2EntryRow): ScheduleItemCore {
  return {
    id: row.id,
 ...
 * ヘルパー名の比較は完全一致で行う。
 */
export function buildSub2CoHelpers(row: Sub2EntryRow, allRows: Sub2EntryRow[]): string[] {
  if (!row.start_time) {
    return [];
  }

  const myName = String(row.helper_name ?? "").trim();
  const coHelpers = new Set<string>();

  for (const other of allRows) {
    if (other === row) continue;
    if (!other.start_time) continue;
    if (other.start_time !== row.start_time) continue;
    if (other.user_name !== row.user_name) continue;

    const otherName = String(other.helper_name ?? "").trim();
    if (otherName === "" || otherName === myName) continue;
 ...
 */
export async function fetchSub2ScheduleByHelperEmail(
  helperEmail: string,
  date: string,
  logLabel: string,
): Promise<ScheduleItemWithCoHelpers[]> {
...
/** 指定日の全ヘルパー分の予定（全体一覧用） */
export async function fetchSub2ScheduleAllByDate(date: string): Promise<ScheduleItemCore[]> {
  const rows = await fetchSub2EntriesByDate(date);
  return rows.map(toScheduleItemFromSub2);
}

export interface Sub2SummaryRow {
  name: string | null;
  helper_email: string;
  start_time: string | null;
}
 ...
 * 対象のヘルパー名を warn に出す。
 */
export async function fetchSub2SummaryRowsByDate(
  date: string,
  logLabel: string,
): Promise<Sub2SummaryRow[]> {
  ...
  const summaryRows: Sub2SummaryRow[] = [];
  ...
  if (missingNames.size > 0) {
    console.warn(
      `[${logLabel}] sub2 helper にメール未登録のため除外しました date=${date} helpers=${Array.from(missingNames).join(" / ")}`,
    );
  }
...
export async function fetchSub2NextEntryOnDate(
  helperNames: string[],
  date: string,
  currentTime: string,
): Promise<Sub2EntryRow | null> {
  if (helperNames.length === 0) {
    return null;
  }
  ...
  return ((data ?? [])[0] ?? null) as Sub2EntryRow | null;
}
 ...
 */
export async function fetchSub2NextEntryAfterDate(
  helperNames: string[],
  afterDate: string,
): Promise<Sub2EntryRow | null> {
  if (helperNames.length === 0) {
