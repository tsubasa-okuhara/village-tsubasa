...
export async function handleAdminPending(
  req: Request,
  res: Response<PendingSuccess | PendingError>
): Promise<void> {
  if (req.method !== "GET") {
  ...
  try {
    ...
    const helperEmails = Array.from(
      new Set(claims.map((c) => String(c.helper_email).toLowerCase()))
    );

    const { data: scheduleRows, error: scheduleErr } = await supabase
      .from("schedule")
      .select(
        "id, date, client, start_time, end_time, task, haisha, summary, beneficiary_number, helper_email, deleted_at"
      )
      .in("id", scheduleIds);
