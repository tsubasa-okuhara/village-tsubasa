...
export async function handleAdminApprove(
  req: AdminRequest,
  res: Response<ApproveSuccess | ApproveError>
): Promise<void> {
  if (req.method !== "POST") {
  ...
  try {
    ...
    if (scheduleUpdateErr) {
      console.error(
        "[self-matching/admin/approve] schedule update error, attempting rollback:",
        scheduleUpdateErr
      );
      await supabase
        .from("schedule_claims")
    ...
    if (!scheduleUpdated) {
      console.warn(
        "[self-matching/admin/approve] schedule.helper_email already set; rolling back claim"
      );
      await supabase
        .from("schedule_claims")
        .update({ status: "pending", decided_at: null, decided_by: null })
    ...
    if (rejectErr) {
      console.error(
        "[self-matching/admin/approve] reject other claims error (non-fatal):",
        rejectErr
      );
    }
