 ...
 */
async function lookupHelperNameByEmail(
 ...
 */
export async function handleHelperLookup(req: Request, res: Response) {
  try {
 ...
 * }
 */
export async function handleSubmitTrainingReport(req: Request, res: Response) {
  try {
    const {
      helperEmail,
      helperName,
      trainingName: bodyTrainingName,
      trainingDate: bodyTrainingDate,
      trainingHours: bodyTrainingHours,
      trainingFormat: bodyTrainingFormat,
      comment,
      trainingMaterialId,
      checklistAnswers,
      extraComments,
    ...
    let materialRecord:
      | {
          ...
          checklist_items: Array<{ id: number; text: string }> | null;
        }
      | null = null;
    ...
    if (trainingMaterialId) {
      const { data: mData, error: mErr } = await supabase
        ...
        .select(
          "id, training_name, training_date, training_hours, training_format, checklist_items, is_active",
        )
    ...
    const finalTrainingName =
      materialRecord?.training_name ??
      (typeof bodyTrainingName === "string" ? bodyTrainingName.trim() : "");
    const finalTrainingDate =
      materialRecord?.training_date ?? bodyTrainingDate ?? null;
    const finalTrainingHours =
      materialRecord?.training_hours ?? bodyTrainingHours ?? null;
    const finalTrainingFormat =
      materialRecord?.training_format ?? bodyTrainingFormat ?? null;
      ...
      if (Array.isArray(checklistAnswers)) {
        for (const a of checklistAnswers) {
          if (a && typeof a === "object") {
            ...
            const checked = Boolean((a as { checked?: unknown }).checked);
            if (Number.isFinite(id)) answersMap[id] = checked;
          }
        }
      }
    ...
    // 感想コメント（最大3つ）の正規化
    let normalizedExtra: string[] | null = null;
    if (Array.isArray(extraComments)) {
      ...
      if (normalizedExtra.length === 0) normalizedExtra = null;
    }

    // helper_master からメールに該当する名前を取得（あれば優先）
    const normalizedEmail =
      typeof helperEmail === "string" && helperEmail.trim()
        ? helperEmail.trim().toLowerCase()
        : null;
    const resolvedHelperName =
      (await lookupHelperNameByEmail(normalizedEmail)) ||
 ...
 */
export async function handleSubmitTrainingNotice(req: Request, res: Response) {
  try {
    const {
      email,
      trainingName,
 ...
 */
export async function handleGetTrainingReports(req: Request, res: Response) {
  try {
    ...
    let query = supabase
      .from("training_reports")
      .select(
        "id, report_type, training_name, training_date, training_hours, training_format, helper_email, helper_name, original_comment, ai_comment, admin_email, notice_body, training_material_id, checklist_answers, extra_comments, status, created_at",
      )
 ...
 * Body: { id, email }
 */
export async function handleDeleteTrainingReport(
  req: Request,
  res: Response,
) {
  try {
 ...
 */
export async function handleUpdateTrainingReportStatus(
  req: Request,
  res: Response,
) {
  try {
