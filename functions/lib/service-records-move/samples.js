...
function byServiceDateDesc(a, b) {
 ...
 */
async function fetchLegacySamples(userName) {
    const supabase = (0, supabase_1.getSupabaseClient)();
    const base = supabase
        .from("service_notes_move")
        .select("service_date, task, summary_text");
    const { data, error } = await (userName.endsWith("様")
...
async function handleSamplesMove(req, res) {
    ...
    try {
        const [legacyResult, sub2Result] = await Promise.allSettled([
            ...
            (0, serviceRecordsSub2_1.fetchSub2MoveSamples)(userName, FETCH_LIMIT),
        ]);
        ...
        const samples = merged
            ...
            service_date: row.service_date ?? "",
            task: row.task ?? "",
            note: (0, noteTimePrefix_1.stripNoteTimePrefix)(row.body ?? ""),
        }))
