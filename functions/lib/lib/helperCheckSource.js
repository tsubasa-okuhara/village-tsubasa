...
exports.fetchConfirmedScheduleRange = fetchConfirmedScheduleRange;
exports.fetchConfirmedThrough = fetchConfirmedThrough;
...
const SELECT_COLUMNS = "id, date, helper_name, user_name, start_time, end_time, transport, support_flow, helper_note, updated_at";
/**
 ...
 */
async function fetchConfirmedScheduleRange(startDate, endDate) {
 ...
 */
async function fetchConfirmedThrough() {
    const supabase = (0, supabase_1.getSupabaseSub2Client)();
    const { data, error } = await supabase
        .from(TABLE)
        .select("date")
...
function toConfirmedScheduleItem(row) {
    return {
        id: row.id,
        date: row.date,
